...
contract ProtocolFactoryController {
    ...
    event Protocol_Router_Factory_Created(address indexed factory, address indexed owner, address yieldToken, address principalToken);

    constructor(address _addressProvider) {
        s_implementation = address(new ProtocolRouterFactory());
        s_addressesProvider = IPoolAddressesProvider(_addressProvider);
        s_factoryControllerOwner = msg.sender;
    }

    function createProtocolRouterFactory(
        address _factoryOwner,
        uint256 _startingRouterFeePercentage,
        address _yieldBarringToken,
        address _principalToken
    ) external returns (ProtocolRouterFactory) {
        address clone = Clones.clone(s_implementation);
        ProtocolRouterFactory factory = ProtocolRouterFactory(clone);

        factory.initialize(
            address(s_addressesProvider),
            address(this),
            _factoryOwner,
            _startingRouterFeePercentage,
            _yieldBarringToken,
            _principalToken
        );
        ...
        emit Protocol_Router_Factory_Created(address(factory), _factoryOwner, _yieldBarringToken, _principalToken);
        return (factory);
    }
}
